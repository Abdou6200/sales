...
export const BonFormModal = ({ onClose, onSuccess, existingBon }: BonFormModalProps) => {
  ...
  useEffect(() => {
    ...
    if (existingBon) {
      setFormData({
        title: existingBon.title,
        code: existingBon.code,
        remise: existingBon.remise,
        duree: existingBon.duree,
        description: existingBon.description,
        partner: existingBon.partner,
      });
    }
  }, [existingBon]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const token = localStorage.getItem("token");
    const url = existingBon
      ? `http://localhost:3000/api/bonreduction/${existingBon._id}`
      : "http://localhost:3000/api/bonreduction";
    const method = existingBon ? "PUT" : "POST";
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      ...
      >
        ...
        <h2 className="text-lg font-semibold mb-4 text-center">
          {existingBon ? "Edit Bon de Réduction" : "Add New Bon de Réduction"}
        </h2>
